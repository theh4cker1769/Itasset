...
const ReturnedAssets = ({sidebarOpen}) => {
    ...
    useEffect(() => {
        const fetchReturnedAssets = async () => {
            try {
                const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/api/return-assets?user_id=${userID}&company_id=${companyID}`)
                const data = await response.json()
                setReturnedAssets(data.data);
            } catch (error) {
    ...
    const currentAssets = returnedAssets.slice(indexOfFirstEntry, indexOfLastEntry);

    const pageNumbers = [];
    for (let i = 1; i <= Math.ceil(returnedAssets.length / entriesPerPage); i++) {
        pageNumbers.push(i);
    }
                            ...
                            <table className="checkbox-datatable table nowrap dataTable no-footer dtr-inline addlist-table" id="DataTables_Table_3" role="grid" aria-describedby="DataTables_Table_3_info">
                                ...
                                <tbody>
                                    {currentAssets.map((v, i) => (
                                        <tr role="row" className="odd assign-asset-table" key={v.returnasset_id}>
                                            ...
                                            <td>{v.return_description}</td>
                                            <td>{v.return_date.slice(0, 10)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
                    ...
                    <div className="row mt-3">
                        <div className="col-sm-12 col-md-5">
                            <div className="dataTables_info" id="DataTables_Table_3_info" role="status" aria-live="polite">
                                Showing {indexOfFirstEntry + 1} to {Math.min(indexOfLastEntry, returnedAssets.length)} of {returnedAssets.length} entries
                            </div>
                        </div>
